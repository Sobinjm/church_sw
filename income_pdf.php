...
{
    $query2=$query.' where type=1 AND status=0';
    $query_par2=' where type=1 AND status=0 ';
}
...
{
    $query1=$query.' where type=0 AND status=0';
    $query_par1=' where type=0 AND status=0 ';
}
...
$pdf->Cell(121.5,8,'Income',1,0,'L');
$pdf->Cell(57.5,8,'Amount',1,0,'C');
$income_total=0;
// echo $query_main;
...
$category_amount=getCategoryAmount($query_par1,$row['category']);
$pdf->SetFont('tamil','',12);
// echo $category_name;
$pdf->Cell(121.5,8,htmlspecialchars($category_name),1,0,'L');
$pdf->SetFont('Arial','',12);
$pdf->Cell(57.5,8,moneyFormatIndia($category_amount).'.00',1,0,'R');
...
// echo "SELECT pr.*,fl.t_area,fl.family_head,fl.head_id from porul_receipt pr inner join family_list fl on pr.family_id=fl.family_id ".$porul_date." order by id ";
// die();
while ($row = $results->fetchArray()) {
$var=12;
...
$pdf->Ln();
$pdf->SetFont('Arial','',12);
$pdf->Cell(12,8,$k,1,0,'C');
$pdf->SetFont('tamil','',12);
$pdf->Cell(121.5,8,'kndhfur; re;ij',1,0,'L');
$pdf->SetFont('Arial','',12);
$pdf->Cell(57.5,8,moneyFormatIndia($porul_amount).'.00',1,0,'R');

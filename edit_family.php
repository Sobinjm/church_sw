...
<?php include('breadcrumb.php'); include('db_info.php');
$i=1;
$id=$_GET['id'];
$results = $db->query("SELECT fm.id as  mem_id,* from family_members fm inner join family_list fl on fm.family_id=fl.family_id  where fm.family_id='$id'");

$results1 = $db->query("SELECT count(*),fm.id as  mem_id,* from family_members fm inner join family_list fl on fm.family_id=fl.family_id  where fm.family_id='$id'");
?>
...
<form action="controls/update_family.php" name="family_form" method="POST" enctype="multipart/form-data">
...
?><input id="text" type="text"  style="display: none;" placeholder=" Enter something...">
            ...
            <tr>
                ...
                <td>
                   <span><?php echo $status.' '.$title.' '.$ini.' '.$ename.'('.$mem_id.')'; ?></span>
                </td>
                <td>
                ...
                <td>
                    ...
                    <!-- <script>alert('<?php echo $member_id;?>');</script> -->
...
<script type="text/javascript">
    window.location.replace("<?php echo 'family_list.php?id='.$id; ?>");
</script>
    <?php
    // die();
    // header('location:family_list.php?id='.$id);
...
?>
        ...
        <a  class="btn btn-primary btn-lg mt-2 column col-xs-3 centered" href="add_member.php?id=<?php echo $id;?>" style="width: 10%;">Add Member </a>
        <a style="width: 10%;" class="btn btn-primary btn-lg mt-2 centered" href="#delete_modal">Delete Family</a>
    </form>
...
function getPlace($place)
{
 ...
 $results = $db->query("SELECT distinct * from place ;");
  ...
  while ($row = $results->fetchArray()) {
    ...
    $op1.='<option '.$check.' value="'.$row['id'].'">'.$row['tamil'].'</option>';
}
$db->close();
return $op1;
}

...
if($_GET['category']==39)
{
    ...
    header('location:sunday_porul_pdf.php?category=39&from='.$_GET['from'].'&to='.$_GET['to']);
}
...
function Footer()
{
 ...
 // echo $query;
$results = $db->query($query.' order by fl.family_order ASC, p.english ASC');
// $db = new dbObj();
...
$pdf->Cell($len*2,8,$ini,'L,T,B');
$pdf->SetFont('tamil','',11);
$pdf->Cell(60+abs($len2),8,' '.$name,',T,B');
$pdf->Cell(59,8,$place,1);
$pdf->SetFont('Arial','',11);
$pdf->Cell(20,8,$row['description'],1);
$pdf->Cell(25,8,moneyFormatIndia($row['amount']).'.00',1,0,'R');
$total+=$row['amount'];
}
$i++;

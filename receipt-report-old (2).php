...
if ($_POST['category']==25) {
    header('location:porul_pdf.php?date='.$_POST['from']);
    # code...
}
// else{
//     $query=$query_main;
...
function Header()
{
    ...
    {
        $this->Cell(0,10,'        '.'rp.v!;.I. nrfu rig KGf;nfhL'.'     '.$_POST['from'].' - '.$_POST['to'],0);
    // Line break
        
    }
    elseif ($_POST['from']!='' && $_POST['to'] =='') {
        $this->Cell(0,10,'             '.'rp.v!;.I. nrfu rig KGf;nfhL'.'     '.$_POST['from'],0);
$this->Ln(5);
        $this->Cell(0,10,$_POST['from'],0);
    // Line break
       
        # code...
    }
    elseif ($_POST['from']=='' && $_POST['to'] !='') {
        $this->Cell(0,10,'             '.'rp.v!;.I. nrfu rig KGf;nfhL'.'     '.$_POST['to'],0);
    // Line break
    ...
    $this->Ln();
    $cat_name=getCatName($_POST['category']);
...
function Footer()
{
 ...
 // echo $query;
$results = $db->query($query.' order by p.english');
// $db = new dbObj();
...
$pdf->Cell(12,8,'','L,T,B');
$pdf->Cell(50,8,'Name','R,T,B');
$pdf->Cell(49,8,'Place',1,0,'C');
$pdf->Cell(20,8,'Months',1,0,'C');
...
$pdf->Cell(50+abs($len2),8,' '.$name,',T,B');
$pdf->Cell(49,8,$place,1);
$pdf->SetFont('Arial','',11);
$pdf->Cell(20,8,$row['months'],1);
